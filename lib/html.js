export function generateHTML(title, body, cssPath = 'src/global.css', scriptPath = 'src/main.js', language = 'en', version = '1.0.0', report = null, reportText = 'Report an issue') {
  ...
  const isEmail = report && /^[^@]+@[^@]+\.[^@]+$/.test(report);
  const reportUrl = report ? (isEmail ? `mailto:${report}` : report) : null;
  const reportLink = reportUrl ? `<a href="${reportUrl}" ${reportUrl.startsWith('mailto:') ? '' : 'target="_blank" rel="noopener"'}>${reportText}</a>` : '';
